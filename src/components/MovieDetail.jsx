...
if (loading) return (
  <>
    <Header />
    <DotLottieReact src="https://lottie.host/4824a7f8-5196-445f-94bd-b0db54471d6a/V0HkJ56JpA.lottie" loop autoplay />
  </>
);
if (!data) return null; 
  ...
  <div className='w-screen'>
    ...
    <div className="pl-4 max-w-screen">
        <div className="bg-gray-900 text-white rounded-2xl shadow-lg border border-gray-800 p-6 flex md:flex-row flex-col gap-6">
          <img src={movieDetails.Poster} alt={movieDetails.Title} className=" rounded-xl max-w-86 shadow-md"/>
        <div>
          <h1 className="text-3xl font-bold">{movieDetails.Title}</h1>
          <p className="text-gray-400">{movieDetails.Year} • {movieDetails.Genre}</p>
          <p className="mt-3">{movieDetails.Plot}</p>
          {movieDetails.imdbRating && movieDetails.imdbVotes && (
            <p className="mt-2 text-yellow-400 font-semibold">⭐ {movieDetails.imdbRating} / 10 ({movieDetails.imdbVotes} votes)</p>
          )}
          ...
          {movieDetails.Rated && (
            <p className="mt-2 text-gray-400"><span className="text-white">Rated:</span> {movieDetails.Rated}</p>
          )}
          ...
          <button onClick={() => setShowModal(true)} className="mt-4 px-4 py-2 bg-blue-600 rounded-xl hover:bg-blue-700 transition" >Add Review</button>
        </div>
      </div>

      <h2 className="mt-8 text-4xl text-white font-bold">Reviews</h2>
      <div className="mt-4 space-y-4 mb-10 w-full max-w-3xl">
        {reviews.length === 0 ? (
          ...
          reviews.map((rev) => (
            <div key={rev._id} className="bg-gray-100 p-4 rounded-xl flex gap-4 items-start">
          ...
          <div className="fixed inset-0 flex items-center justify-center backdrop-blur-md bg-opacity-50">
            <div className="bg-white p-6 rounded-2xl w-96 shadow-xl">
              ...
              <div className="flex gap-2 mb-4">
                {Array.from({ length: 5 }, (_, i) => (
                  <Star key={i} size={28} onClick={() => setRating(i + 1)}
                    onMouseEnter={() => setHover(i + 1)}
                    onMouseLeave={() => setHover(0)}
                    className={`cursor-pointer ${(hover || rating) > i ? "fill-yellow-500 text-yellow-500" : "text-gray-400"}`}
                  />
                ))}
              </div>

              <textarea required className="w-full border rounded-lg p-2" rows="3" placeholder="Write your review..." value={comment} onChange={(e) => setComment(e.target.value)}></textarea>
              <p className="text-red-500">{errorPop}</p>
              <div className="flex justify-end gap-3 mt-4">
                <button onClick={() => setShowModal(false)} className="px-4 py-2 rounded-lg border cursor-pointer">Cancel</button>
                <button type="submit" onClick={handleAddReview} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 cursor-pointer">Post Review</button>
              </div>
            </div>
          </div>
        )}
